...
class ValidateCode
{
    ...
    //用图片生成背景
    private function createImageBg($path = '/view/default/images/bg_code')
    {
        $path .= mt_rand(1, 3) . '.png';
        $this->img = imagecreatefrompng(ROOT_PATH . $path);
    }
    ...
    private function createImageFont()
    {
        $_x = $this->width / $this->codelen;
        for ($i = 0; $i < $this->codelen; $i++) {
    ...
    {
        $this->createImageBg(); // $this->createBg();
        $this->createCode();
        //$this->createLine();
        $this->createImageFont(); //$this->createFont();
        $this->outPut();
    }
