                ...
                <tr>
                    ...
                    <td>
                        {if $value->is_up == 0}
                            ...
                            <a href="?a=goods&m=isUp&act=up&id={$value->id}">[上架]</a>
                        {else}
                            <span class="green">是</span>
                            <a href="?a=goods&m=isUp&act=down&id={$value->id}">[下架]</a>
                        {/if}
                    </td>
                    <td>{$value->inventory}</td>
                    <td>
                        <a href="?a=goods&m=update&id={$value->id}"><img src="/view/admin/images/edit.gif" alt="编辑"
                                                                         title="编辑"/></a><a
                                href="?a=goods&m=delete&id={$value->id}"
                                onclick="return confirm('你真的要删除这个商品么?') ? true : false"><img
                                    src="/view/admin/images/drop.gif" alt="删除" title="删除"/></a>
                    </td>
                </tr>
                {foreachelse}
                <tr>
                    <td colspan="8">没有任何商品！</td>
                </tr>
            {/foreach}
        </table>
    </form>
